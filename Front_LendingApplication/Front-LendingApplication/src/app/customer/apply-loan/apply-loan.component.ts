...
export class ApplyLoanComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.customerService.getLoanSchemes().subscribe({
      next: (schemes) => {
        this.loanSchemes = schemes;
        this.loadSchemeFromQuery(); // ✅ LOAD schemeId after loan schemes are fetched
      },
      error: (err: Error) => {
        this.errorMessage = `Failed to load loan schemes: ${err.message}`;
      }
  ...
  // ✅ ADD THIS FUNCTION
  loadSchemeFromQuery(): void {
    this.route.queryParams.subscribe(params => {
      const schemeId = params['schemeId'];
      if (schemeId) {
        console.log('Scheme ID received from URL:', schemeId);
        this.applyForm.patchValue({ schemeId: Number(schemeId) });
        this.onSelectScheme(); // Automatically load documents too
      }
    });
  }
        ...
        next: (loan) => {
          ...
          this.customerService.updateDocumentLoanIds(this.uploadedDocumentIds, loan.loanId).subscribe({
            next: () => {
              ...
              setTimeout(() => {
                this.router.navigate(['/customer/view-loans']).then(success => {
                  console.log('Navigation to /customer/view-loans successful:', success);
                  if (!success) {
                    console.error('Navigation failed, redirected to:', this.router.url);
                  }
                });
