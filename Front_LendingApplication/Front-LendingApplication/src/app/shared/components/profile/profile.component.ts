...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (role === 'ROLE_ADMIN') {
      ...
      if (loanOfficerId) {
        this.loadLoanOfficerProfile(loanOfficerId);
      } else {
        this.errorMessage = 'Loan Officer profile not found.';
      }
    } else {
  ...
  loadLoanOfficerProfile(loanOfficerId: number): void {
    this.profileService.getLoanOfficerProfile(loanOfficerId).subscribe({
      next: profile => {
        this.profile = profile;
        this.updateForm.patchValue({
          firstName: profile.firstName,
  ...
  updateProfile(): void {
    ...
    } if (role === 'ROLE_LOAN_OFFICER') {
      id = this.authService.getLoanOfficerId();
    } else if (role === 'ROLE_CUSTOMER') {
