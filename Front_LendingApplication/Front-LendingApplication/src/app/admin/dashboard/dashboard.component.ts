    ...
    this.loanOfficerService.getAllLoanOfficers().subscribe({
      next: officers => {
        ...
        if (officers.length === 0) {
          this.errorMessage = 'No loan officers found. Add one to get started.';
        }
      },
      error: err => {
        console.error('Fetch loan officers count failed:', err);
        this.loanOfficerCount = 0;
        this.errorMessage = err.status === 0
          ? 'Server unreachable. Please check your connection or CORS settings.'
          : 'Failed to load loan officers. Please try again or add a loan officer.';
      }
    });
    this.loanSchemeService.getAllLoanSchemes().subscribe({
      next: schemes => {
        this.loanSchemeCount = schemes.filter(s => !s.isDeleted).length;
        if (schemes.length === 0 && !this.errorMessage) {
          this.errorMessage = 'No loan schemes found. Add one to get started.';
        }
      },
      error: err => {
        console.error('Fetch loan schemes count failed:', err);
        this.loanSchemeCount = 0;
        if (!this.errorMessage) {
          this.errorMessage = err.status === 0
            ? 'Server unreachable. Please check your connection or CORS settings.'
            : 'Failed to load loan schemes. Please try again.';
        }
      }
    });
  }
