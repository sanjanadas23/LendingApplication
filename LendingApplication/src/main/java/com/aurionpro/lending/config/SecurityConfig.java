package com.aurionpro.lending.config;
...
import com.aurionpro.lending.security.JwtAuthenticationFilter;
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.cors(cors -> cors.configurationSource(corsConfigurationSource())).csrf(csrf -> csrf.disable())
				...
				.authorizeHttpRequests(auth -> auth.requestMatchers("/api/users/registerUser", "/api/users/login")
						...
						.requestMatchers("/api/loan-schemes/*/document-types").hasRole("CUSTOMER")
						.requestMatchers("/api/loan-schemes/**").hasRole("ADMIN")
						.requestMatchers(HttpMethod.GET, "/api/loan-payments/totalPaymentAmount/*/amount")
						.hasRole("CUSTOMER").requestMatchers(HttpMethod.POST, "/api/loan-payments/*/repay")
						.hasRole("CUSTOMER").requestMatchers(HttpMethod.POST, "/api/loan-payments/complete")
