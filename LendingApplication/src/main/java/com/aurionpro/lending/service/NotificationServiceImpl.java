...
import com.aurionpro.lending.exception.NotificationException;
...
public class NotificationServiceImpl implements NotificationService {
	...
	public void sendLoanStatusEmail(int loanId, String status) {
		...
		if (customerUserOpt.isEmpty()) {
			throw new ResourceNotFoundException("Customer user not found for loan ID: " + loanId);
		}
		...
		try {
			...
			throw new NotificationException("Failed to send loan status email: " + e.getMessage());
		}
	}
	...
	public void sendPaymentReminderEmail(int loanPaymentId) {
		...
		Optional<User> customerUserOpt = userRepository.findById(customer.getUser().getId());
		if (customerUserOpt.isEmpty()) {
			throw new ResourceNotFoundException("Customer user not found for loan ID: " + loan.getLoanId());
		}
		User customerUser = customerUserOpt.get();
		...
		try {
			...
			throw new NotificationException("Failed to send payment reminder email: " + e.getMessage());
		}
	}
	...
	public void sendInstallmentPlanEmail(int loanId) {
		...
		try {
			...
			throw new NotificationException("Failed to send installment plan email: " + e.getMessage());
		}
	}
	...
	public void sendPaymentConfirmationEmail(int loanPaymentId, BigDecimal amountPaid) {
		...
		try {
			...
			throw new NotificationException("Failed to send payment confirmation email: " + e.getMessage());
		}
	}
	...
	public void sendNpaPendingNotificationToOfficer(int loanId, int loanOfficerId) {
		...
		if (officerUserOpt.isEmpty()) {
			throw new ResourceNotFoundException("Loan officer user not found for ID: " + loanOfficerId);
		}
		...
		try {
			...
			throw new NotificationException("Failed to send NPA pending notification: " + e.getMessage());
		}
	}
	...
	public void sendNpaNotificationToCustomer(int customerId, int loanId, BigDecimal totalDues) {
		...
		Optional<User> customerUserOpt = userRepository.findById(customer.getUser().getId());
		if (customerUserOpt.isEmpty()) {
			throw new ResourceNotFoundException("Customer user not found for ID: " + customerId);
		}
		User customerUser = customerUserOpt.get();
		...
		try {
			...
			throw new NotificationException("Failed to send NPA notification to customer: " + e.getMessage());
		}
	}

	public void sendDocumentRejectionEmail(int loanId, Document document) {
		Optional<Loan> loanOpt = loanRepository.findById(loanId);
		if (loanOpt.isEmpty()) {
			throw new ResourceNotFoundException("Loan not found with ID: " + loanId);
		}
		...
		try {
			...
			message.setFrom(new InternetAddress(smtpUsername));
			message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(customerUser.getEmail()));
			message.setSubject("Document Rejection Notification");
			message.setText(String.format(
					"Dear %s,\n\nYour document '%s' (ID: %d) for Loan ID: %d has been rejected by the loan officer. "
							+ "As a result, your loan application has been rejected. "
							+ "Please reupload the corrected document for re-verification.\n\nRegards,\nLoan Management Team",
					customerUser.getUsername(), document.getDocumentName(), document.getDocumentId(), loanId));
			Transport.send(message);
		} catch (MessagingException e) {
			throw new NotificationException("Failed to send document rejection email: " + e.getMessage());
		}
	}
}